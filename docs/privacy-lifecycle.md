...
`droppedRetries` counts pending events dropped because retries are exhausted **or** a valid server minimum exceeds the per-delay `retryMaxMs` limit. This is not a total elapsed-time budget or a Cloud quota reason. Accepted/duplicate/permanently rejected IDs are already removed and are not counted again. The unchanged aggregate counter intentionally contains no header/body/error details; see `contracts.md` for the Retry-After policy. TR: `droppedRetries`, deneme sayısı tükenen veya server minimumu tek bekleme sınırını aşan bekleyen olayları sayar; toplam süre/kota anlamı taşımaz. Daha önce kararı verilmiş olaylar ikinci kez sayılmaz, ham içerik raporlanmaz.
