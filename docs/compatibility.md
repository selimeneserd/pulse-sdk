...
Research and local verification date: 2026-09-11. Both complete checks and packed gates passed locally on macOS arm64; the Ubuntu CI matrix is configured but was not executed remotely in this task. The installed package metadata and README identify `@modelcontextprotocol/server`, `client`, `node` and `core` as **2.0.0** (upstream package engines `node >=20`). Pulse's narrower support is determined by its own tests, not inherited from upstream's engine range. The exact peer pin remains because no second MCP release has been installed and tested. This is a support declaration, not a runtime package-version lock.

Araştırma ve yerel doğrulama tarihi: 2026-09-11. İki runtime için tam kontrol ve paket testleri macOS arm64 üzerinde geçti; Ubuntu CI matrisi yapılandırıldı, bu görevde uzaktan çalıştırılmadı. Kurulu MCP paketleri **2.0.0**. Pulse desteği upstream'in geniş runtime aralığından değil, kendi testlerinden gelir. Başka MCP sürümü gerçekten kurulup test edilmediği için peer sürümü 2.0.0 kalır; çalışma zamanında paket sürümü kilidi uygulanmaz.

| Surface / Yüzey | Evidence / Kanıt |
| --- | --- |
| Node 24.11.1, Node 24.20.0 + MCP 2.0.0 | Full suite: 133/133 on each runtime, including 33 adapter/HTTP/CLI tests. CI is configured for both versions; packed-run evidence is recorded in `docs/evidence/packed-consumer-node24.11.1.json` and `docs/evidence/packed-consumer.json`. |
